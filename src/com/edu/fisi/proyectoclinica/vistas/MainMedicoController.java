...
public class MainMedicoController extends Ventana implements Initializable {
    ...
    private ArrayList<HistoriaClinicaTO> hs;
    int indiceActual=0;
    @FXML
...
//        txtTelefono.setText(paciente.getTelefono());
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        hs.addAll(hisRep.readDNI(c.getPaciente().getDni()));
        set(hs.get(indiceActual));
    }
    public void siguiente(ActionEvent e){
        if(hs.size()<indiceActual-1){
            indiceActual++;
            hs.add(hs.get(indiceActual));
        }
    }
    public void atras(ActionEvent e){
        if(hs.size()-1>indiceActual){
            indiceActual--;
            hs.add(hs.get(indiceActual));
        }
    }
